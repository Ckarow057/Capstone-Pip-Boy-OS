...
import pygame
import theme
from config import CONTENT_TOP, INVENTORY_CATEGORIES
...
def draw(surface, ui, app_state):
    ...
    # Vertical divider
    pygame.draw.line(surface, theme.PIP_GREEN, (210, CONTENT_TOP + 40), (210, ui.height - 80), 1)

    # ----- Item list (all items for now) -----
    list_x = 230
    col_headers_y = CONTENT_TOP + 45
    col_name_x = list_x
    col_wt_x = list_x + 320
    ...
    for label, x in [("NAME", col_name_x), ("WT", col_wt_x), ("VAL", col_val_x), ("QTY", col_qty_x)]:
        h_surf = ui.fonts.tiny.render(label, True, theme.PIP_GREEN_DIM)
        surface.blit(h_surf, (x, col_headers_y))

    pygame.draw.line(surface, theme.PIP_GREEN_DARK, (list_x, col_headers_y + 16), (ui.width - 40, col_headers_y + 16), 1)
    ...
    for i, item in enumerate(inventory[:max_visible]):
        ...
        if i % 2 == 0:
            pygame.draw.rect(surface, theme.ROW_ALT_BG, (list_x - 4, row_y - 2, ui.width - list_x - 30, row_h - 2), 0)

        name_surf = ui.fonts.small.render(item["name"], True, theme.PIP_GREEN)
        surface.blit(name_surf, (col_name_x, row_y))

        wt_surf = ui.fonts.small.render(f"{item['weight']:.1f}", True, theme.PIP_GREEN_DIM)
        surface.blit(wt_surf, (col_wt_x, row_y))

        val_surf = ui.fonts.small.render(str(item["value"]), True, theme.PIP_GREEN_DIM)
        surface.blit(val_surf, (col_val_x, row_y))

        qty_surf = ui.fonts.small.render(str(item["count"]), True, theme.PIP_GREEN_BRIGHT)
    ...
    footer = ui.fonts.small.render(
        f"TOTAL WT: {total_weight:.1f}  |  CAPS: {caps}", True, theme.PIP_GREEN_DIM
    )
